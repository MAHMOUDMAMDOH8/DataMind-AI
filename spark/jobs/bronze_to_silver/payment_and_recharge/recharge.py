...
def transform_recharge(df, metadata_endpoint: str = "http://minio:9000"):
    df = normalize_columns(df, "billing_info", "")

    final_df = add_rejection_reason(
        df,
        required_columns=[
            "event_type", "sid", "customer", "recharge_amount",
            "balance_before", "balance_after", "payment_method",
            "status", "timestamp", "phone_number", "transaction_id",
            "amount", "currency",
        ],
        numeric_columns=["recharge_amount", "amount", "balance_before", "balance_after"],
        positive_columns=["recharge_amount"],
    )
    ...
    write_pipeline_metadata_event(
        ...
        entity="recharge.recharges",
        action="transform",
        row_count=valid_count,
        target="local.silver.recharges",
        status="success",
        extra={
            "raw_count": raw_count,
